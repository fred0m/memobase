...
- **SC-001（T-D 回归闸门，2026-08-30 修订，沫沫签核）**: 36 条基准在混合链路（flag 开、真实预算：单日窗 700/多日窗 350 + 事件段 900）上重跑，双指标判定：
  1. **事件段 MRR@10 不低于同口径对照 −0.05**（各场景：auto ≥ 0.658、active ≥ 0.773、temporal ≥ 0.617、entity ≥ 0.950、vague ≥ 0.525；对照 = 同脚本 --no-summary 同口径）
  2. **概要段日级命中 ≥ 基线**（gold 事件所在日 ∈ 概要日集的比例，temporal ≥ 0.78、整体 ≥ 0.50——2026-08-30 实测基线）
  3. **抽样回答质量不降**（LLM-as-judge：36 条中抽 8-10 条，带概要段注入 vs 不带，回答质量不降）
  任一不达 → 不开 flag。旧闸门（all MRR ≥ 0.85）作废——那是文档粒度（宽松）的预期，事件粒度 v3 基线才 0.769，0.85 是口径错配
